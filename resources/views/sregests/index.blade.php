...
@section('main')
    <div class="container">
        <div class="row">
            ...
            @foreach ($sregests as $item)
                <div class="card col-3 p-2 m-2">
                    <div class="card-title text-center ">
                        طلب استرداد ايقاف تسجيل 
                    </div>
                    <div class="bg-gray rounded-sm p-1 m-1">الجنسية : {{$item->nation}}</div>
                    <div class="bg-gray rounded-sm p-1 m-1">العام الدراسي : {{$item->date}}</div>
                    <div class="bg-gray rounded-sm p-1 m-1">سنة التوقف : {{$item->stopseason}}</div>
                    <div class="bg-gray rounded-sm p-1 m-1"> رقم البطاقة الشخصية: {{$item->pId}}</div>
                    
                    @if ($item->st == 'pendding')
                        <div class="bg-warning rounded-sm p-1 m-1">status  :{{$item->st}}</div>
                        <form action="{{route('sregests.update',$item->id)}}" method="post">
                            @csrf
                            @method('patch')
