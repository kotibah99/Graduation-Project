...
@section('main')
    <div class="container">
        <div class="row">
            ...
            @foreach ($fund1s as $item)
                <div class="card col-3 p-2 m-2">
                    <div class="card-title text-center ">
                        طلب تقسيط رسوم التسجيل
                    </div>
                    <div class="bg-gray rounded-sm p-1 m-1">نوع التسجيل : {{$item->type}}</div>
 
                    @if ($item->st == 'pendding')
                        <div class="bg-warning rounded-sm p-1 m-1">status  :{{$item->st}}</div>
                        <form action="{{route('fund1s.update',$item->id)}}" method="post">
                            @csrf
                            @method('patch')
