...
@section('main')
    <div class="container">
        <div class="row">
            ...
            @foreach ($exam1s as $item)
                <div class="card col-3 p-2 m-2">
                    <div class="card-title text-center ">
                        طلب تقدم للدورة التكميلية
                    </div>
                    <div class="bg-gray rounded-sm p-1 m-1">عدد المقررات : {{$item->number}}</div>
                    <div class="bg-gray rounded-sm p-1 m-1">اسماء المقررات :{{$item->itemsnames}}</div>
                    @if ($item->st == 'pendding')
